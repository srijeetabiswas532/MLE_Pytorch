...
class Network(minitorch.Module):
    ...
    def __init__(self):
        ...
        self.linear2 = Linear(64, 10)  # 10 classes
        # raise NotImplementedError("Need to implement for Task 4.5")

    def forward(self, x):
        # TODO: Implement for Task 4.5.
        x1 = self.conv1.forward(x).relu()
        x2 = self.conv2.forward(x1).relu()

        l1 = self.linear1.forward(minitorch.avgpool2d(x2, (4, 4)).view(BATCH, 392)).relu()

        if self.training:
            l1 = minitorch.dropout(l1, 0.25)
        res = self.linear2.forward(l1)

        return minitorch.logsoftmax(res, dim=1)
        # raise NotImplementedError("Need to implement for Task 4.5")
...
def default_log_fn(epoch, total_loss, correct, total, losses, model, file):
    file.write(f"Epoch {epoch} loss {total_loss} valid acc {correct}/{total}\n")
    # file.write('')
...
class ImageTrain:
    ...
    def train(
        ...
        losses = []
        file = open("/Users/srijeetabiswas/Desktop/Desktop/transfer to mac/torsha/grad school/Fall Semester/courses/MLE/Assignment4/mnist.txt", "w")
                ...
                if batch_num % 5 == 0:
                    ...
                    for val_example_num in range(0, 1 * BATCH, BATCH):
                        ...
                        for i in range(BATCH):
                            ...
                            if y[i, ind] == 1.0:
                                correct += 1
                    log_fn(epoch, total_loss, correct, BATCH, losses, model, file)
