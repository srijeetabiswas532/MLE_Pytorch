...
class CNNSentimentKim(minitorch.Module):
    ...
    def forward(self, embeddings):
        ...
        x4 = (minitorch.nn.max(x1, 2) + minitorch.nn.max(x2, 2) + minitorch.nn.max(x3, 2))
        x5 = self.linear(x4.view(x4.shape[0], x4.shape[1]))
        # x5 = self.linear(x4)

        if self.training:
            x5 = minitorch.dropout(x5, self.dropout)

        # x6 = minitorch.nn.dropout(x6, self.dropout)
        x7 = x5.sigmoid().view(embeddings.shape[0])
        return x7
        # raise NotImplementedError("Need to implement for Task 4.5")
